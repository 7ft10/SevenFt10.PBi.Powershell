...
Function global:Invoke-PBIPowerBIAuthenitcation {
	begin {
...
Function global:Invoke-PBIDataSetsUpload {
	Param
	(
	...
	process {
		Invoke-PBIFileUpload $WorkspaceId $UploadPathFolderOrFile $ConflictAction "DataSet.*.pbix"
	}
...
Function global:Invoke-PBIDataSuitesUpload {
	Param
	(
		[Parameter(Mandatory = $true, Position = 0)]
		[string] $WorkspaceId,
	...
	process {
		Invoke-PBIFileUpload $WorkspaceId $UploadPathFolderOrFile $ConflictAction "DataSuite.*.pbix"
	}
...
Function global:Invoke-PBIReportsUpload {
	Param
	(
		[Parameter(Mandatory = $true, Position = 0)]
		[string] $WorkspaceId,
	...
	process {
		Invoke-PBIFileUpload $WorkspaceId $UploadPathFolderOrFile $ConflictAction "Report.*.pbix"
	}
...
Function Invoke-PBIFileUpload {
	...
	process {
		...
		foreach ($r in $Reports) {
			...
			try {
				New-PowerBIReport -Workspace $Workspace -Path $UploadFile -ConflictAction $ConflictAction -ErrorAction SilentlyContinue -ErrorVariable ProcessError > $null
				If ($ProcessError) {
...
Function global:Remove-PBIDataSetReports {
	[CmdletBinding()]
	param (
	...
	process {
		...
		Foreach ($dataset in $datasets) {
			...
			if ($dataset.IsRefreshable) {
				...
				Invoke-PowerBIRestMethod -Url $url -Method Post -Body $MailFailureNotify -ErrorAction SilentlyContinue -ErrorVariable RefreshFailed > $null
				if ($RefreshFailed) {
					$err = Resolve-PowerBIError -Last
					if ($err.PowerBIErrorInfo) {
...
Function global:Clear-PBIWorkspace {
	[CmdletBinding()]
	param (
		[Parameter(Position = 0)]
		[String]
	...
	process {
		...
		if ($decision -eq 0) {
			...
			Foreach ($dataset in $datasets) {
				...
				Invoke-PowerBIRestMethod -Url $url -Method Delete -ErrorAction SilentlyContinue -ErrorVariable DeleteFailed > $null
				if ($DeleteFailed) {
					$err = Resolve-PowerBIError -Last
					if ($err.PowerBIErrorInfo) {
						Write-Host $err.PowerBIErrorInfo -ForegroundColor Yellow
...
Function global:Update-PBIDebugParameters {
	[CmdletBinding()]
	param (
		[Parameter(Position = 0)]
		[String]
			...
			if ($body.Contains('IsDebug')) {
				...
				Foreach ($param in $parameters.value) {
					if ($param.name -eq "IsDebug" -And $param.type -eq "Logical" -And $param.currentValue -ne $IsDebug) {
						...
						Invoke-PowerBIRestMethod -Url "$url/Default.UpdateParameters" -Method Post -Body $updatebody -ContentType $content > $null
						Write-Host "Done"
						$updated = $true
					}
				}
